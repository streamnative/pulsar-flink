...
public abstract class PulsarTestBase extends TestLogger {
    ...
    public static <T> List<MessageId> sendTypedAutoMessages(
            String topic,
            SchemaType type,
            List<T> messages,
        ...
        try (PulsarClient client = PulsarClient.builder().serviceUrl(getServiceUrl()).build()) {
            switch (type) {
                ...
                case FLOAT:
                    ...
                    break;
                case INT32:
                    producer = client.newProducer(Schema.AUTO_PRODUCE_BYTES(Schema.AVRO(Integer.class))).topic(topicName).create();
                    break;
                case INT16:
            ...
            for (T message : messages) {
                byte[] encode = Schema.AVRO(Integer.class).encode((Integer) message);
                MessageId mid = producer.newMessage().value(encode).send();
