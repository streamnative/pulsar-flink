...
class FlinkPulsarSource(val parameters: Properties)
  ...
  @transient private val taskIndex = getRuntimeContext.getIndexOfThisSubtask

  @transient private val numParallelTasks = getRuntimeContext.getNumberOfParallelSubtasks
  ...
  @transient val successfulCommits: Counter = null
  @transient val failedCommits: Counter = null
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    val allTopics = topicDiscoverer.getTopicPartitions()
  ...
  override def run(sourceContext: SourceFunction.SourceContext[BinaryRow]): Unit = {
    ...
    this.pulsarFetcher = new PulsarFetcher(
      sourceContext,
  ...
  override def notifyCheckpointComplete(checkpointId: Long): Unit = {
    ...
    try {
      ...
      fetcher.commitOffsetToPulsar(offset)
    } catch {
      case e: Exception =>
        if (running) {
          throw e
