...
class PulsarTopicStateWithPeriodicWatermarks(
    topic: String,
    periodicWatermarks: AssignerWithPeriodicWatermarks[GenericRow])
  ...
  def getTimestampForRow(row: GenericRow, timestamp: Long): Long = {
...
class PulsarTopicStateWithPunctuatedWatermarks(
    topic: String,
    punctuatedWatermarks: AssignerWithPunctuatedWatermarks[GenericRow])
  extends PulsarTopicState(topic) {

  /** The last watermark timestamp generated by this partition. */
  @volatile var partitionWatermark: Long = Long.MinValue

  def getTimestampForRow(row: GenericRow, pulsarEventTimestamp: Long): Long = {
    punctuatedWatermarks.extractTimestamp(row, pulsarEventTimestamp)
  }

  def checkAndGetNewWatermark(row: GenericRow, timestamp: Long): Watermark = {
