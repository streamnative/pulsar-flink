...
public class PulsarTableOptions {
    ...
    public static final ConfigOption<WatermarkStrategy> WATERMARK_STRATEGY = ConfigOptions
        .key("watermark.strategy")
        .enumType(WatermarkStrategy.class)
        .noDefaultValue()
        .withDescription("Defines a strategy how to generate watermarks. By default, "
            + "watermarks are generated by the WATERMARK clause of the table definition.");
    ...
    /**
     * Create a watermark strategy based on the table options.
     *
     * @see org.apache.flink.table.connector.source.abilities.SupportsWatermarkPushDown
     */
    public static Optional<org.apache.flink.api.common.eventtime.WatermarkStrategy<RowData>> createWatermarkStrategy(
            ReadableConfig tableOptions) {
        return tableOptions.getOptional(WATERMARK_STRATEGY)
            .flatMap((watermarkStrategy -> {
                switch(watermarkStrategy) {
                    case PULSAR:
                        return Optional.of(PulsarWatermarkStrategy.forPulsarWatermarks());
                    default:
                        return Optional.empty();
                }
            }));
    ...
    /**
     * Strategies for generating watermarks.
     */
