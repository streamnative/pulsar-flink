...
public class PulsarCatalogSupport {
    ...
    @VisibleForTesting
    public PulsarCatalogSupport(PulsarMetadataReader metadataReader,
    ...
    public void createTable(ObjectPath tablePath, ResolvedCatalogTable table) throws PulsarAdminException {
        // only allow creating table in generic database, the topic is used to save table information
        if (!isGenericDatabase(tablePath.getDatabaseName())) {
            throw new CatalogException(String.format(
                "Can't create generic table under pulsar tenant/namespace: %s", tablePath.getDatabaseName()));
        }
