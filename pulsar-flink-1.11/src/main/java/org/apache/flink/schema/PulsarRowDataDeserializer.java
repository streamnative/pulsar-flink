 ...
 */
@Slf4j
public class PulsarRowDataDeserializer implements DeserializationSchema<RowData> {

    private final Function<byte[], RowData> converter;

    private final DataType rootDataType;

    private final FieldsDataType fieldsDataType;

    private final SimpleSchemaTranslator schemaTranslator;
    ...
    public PulsarRowDataDeserializer(DataType producedDataType, SchemaType schemaType, JSONOptions parsedOptions, TypeInformation<RowData> rowDataTypeInfo) {
        this.rowDataTypeInfo = rowDataTypeInfo;
        try {
            schemaTranslator = new SimpleSchemaTranslator(false);
            this.fieldsDataType = getFieldsDataType(producedDataType);
            this.rootDataType = producedDataType;

            final RowRowConverter rowRowConverter = RowRowConverter.create(rootDataType);

            switch (schemaType) {
                case AVRO:
                    FieldsDataType st = (FieldsDataType) rootDataType;
                    int fieldsNum = st.getChildren().size();
                    final org.apache.pulsar.client.api.Schema schema =
                            schemaTranslator.sqlType2PulsarSchema(rootDataType);

                    Schema avroSchema =
                            new Schema.Parser().parse(new String(schema.getSchemaInfo().getSchema(), StandardCharsets.UTF_8));
                    BinFunction<RowUpdater, GenericRecord> writer = getRecordWriter(avroSchema, st, new ArrayList<>());
                    final AvroReader<GenericAvroRecord> avroReader =
                            new AvroReader<>(avroSchema, getClass().getClassLoader(), true);
                    this.converter = msg -> {
                        RowUpdater fieldUpdater = new RowUpdater();
                        GenericRowData resultRow = new GenericRowData(fieldsNum);
                        fieldUpdater.setRow(resultRow);

                        GenericAvroRecord value = avroReader.read(msg, 0, msg.length);
                        writer.apply(fieldUpdater, value.getAvroRecord());
                        return resultRow;
                    };
                    break;
                ...
                case JSON:
                    ...
                    JacksonRecordParser.FailureSafeRecordParser parser = new JacksonRecordParser.FailureSafeRecordParser(
                            ...
                            fdt);
                    this.converter = data -> {
                        int rowSize = fdt.getChildren().size();
                        Row resultRow = new Row(rowSize);
                        parser.parse(new String(data, StandardCharsets.UTF_8), resultRow);
                        return rowRowConverter.toInternal(resultRow);
                    };
                    break;

                default:
                    TriFunction<RowUpdater, Integer, Object> writer2 = newAtomicWriter(rootDataType);
                    final org.apache.pulsar.client.api.Schema pulsarSchema =
                            schemaTranslator.sqlType2PulsarSchema(rootDataType);
                    this.converter = msg -> {
                        RowUpdater fUpdater = new RowUpdater();
                        GenericRowData tmpRow = new GenericRowData(1);
                        fUpdater.setRow(tmpRow);
                        Object value = pulsarSchema.decode(msg);

                        if (schemaType == SchemaType.STRING){
                            value = StringData.fromString((String) value);
                        } else if (schemaType == SchemaType.LOCAL_DATE){
                            value = (int)((LocalDate) value).toEpochDay();
                        } else if (schemaType == SchemaType.LOCAL_TIME){
                            value = ((LocalTime) value).toSecondOfDay();
                        } else if (schemaType == SchemaType.LOCAL_DATE_TIME){
                            value = TimestampData.fromLocalDateTime((LocalDateTime) value);
                        } else if (schemaType == SchemaType.BYTES){
                            value = BinaryArrayData.fromPrimitiveArray((byte[]) value);
                        }
    ...
    public FieldsDataType getFieldsDataType(DataType dataType)
...
//        }
    ...
    private TriFunction<FlinkDataUpdater, Integer, Object> newWriter(Schema avroType, DataType flinkType, List<String> path) throws IncompatibleSchemaException {
        ...
        if (atpe == Schema.Type.NULL && tpe == LogicalTypeRoot.NULL) {
            ...
            return (rowUpdater, ordinal, value) -> {
                GenericRowData row = new GenericRowData(fieldsDataType.getChildren().size());
            ...
            if (!nonNullTypes.isEmpty()) {
                ...
                if (nonNullTypes.size() == 1) {
                    ...
                    if (nonNullTypes.size() == 2) {
                        ...
                        return (updater, ordinal, value) -> {
                            GenericRowData row = new GenericRowData(rt.getFieldCount());
                            RowUpdater fieldUpdater = new RowUpdater();
    ...
    public static final class RowUpdater implements FlinkDataUpdater {
        ...
        public void setRow(GenericRowData currentRow) {
            this.row = currentRow;
        }
