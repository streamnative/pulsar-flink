...
/**
 * Implements the logic around emitting records and tracking offsets, as well as around the optional
 * timestamp assignment and watermark generation.
 *
 * @param <T> The type of elements deserialized from Pulsar messages, and emitted into the Flink
 *     data stream.
 */
@Slf4j
public class PulsarFetcher<T> {
     ...
     * (e.g. current offset, committed offset) and pulsar-shipped metrics will be registered.
     */
    ...
    public PulsarFetcher(
        ...
        this.useEarliestWhenDataLoss =
                SourceSinkUtils.getUseEarliestWhenDataLossAndRemoveKey(this.readerConf);
        ...
        if (timestampWatermarkMode == WITH_WATERMARK_GENERATOR && autoWatermarkInterval > 0) {
            PeriodicWatermarkEmitter<T> periodicEmitter =
    ...
    public void runFetchLoop() throws Exception {
        ...
        try {
            ...
            while (running) {
                ...
                List<PulsarTopicState<T>> topicsToAssign =
                ...
                if (topicToThread.size() == 0 && unassignedPartitionsQueue.isEmpty()) {
                    PulsarTopicState topicForBlocking =
                            unassignedPartitionsQueue.getElementBlocking();
                    if (topicForBlocking.equals(PoisonState.INSTANCE)) {
                        throw BreakingException.INSTANCE;
                    }
                    topicToThread.putAll(
                            createAndStartReaderThread(
                                    ImmutableList.of(topicForBlocking), exceptionProxy));
                }
            }
            ...
            // make sure that in any case (completion, abort, error), all spawned threads are
            // stopped
            try {
    ...
    public void cancel() throws Exception {
        ...
        Set<TopicRange> topics =
                subscribedPartitionStates.stream()
                        .map(PulsarTopicState::getTopicRange)
    ...
    public void addDiscoveredTopics(Set<TopicRange> newTopics)
            throws IOException, ClassNotFoundException {
        List<PulsarTopicState<T>> newStates =
                createPartitionStateHolders(
                        newTopics.stream()
                                .collect(Collectors.toMap(t -> t, t -> MessageId.earliest)),
                        timestampWatermarkMode,
                        watermarkStrategy,
                        userCodeClassLoader);
    ...
    public Map<TopicRange, ReaderThread<T>> createAndStartReaderThread(
            List<PulsarTopicState<T>> states, ExceptionProxy exceptionProxy) {

        Map<TopicRange, MessageId> startingOffsets =
                states.stream()
                        .collect(
                                Collectors.toMap(
    ...
    protected ReaderThread<T> createReaderThread(
            ExceptionProxy exceptionProxy, PulsarTopicState state) {
    ...
    private List<PulsarTopicState<T>> createPartitionStateHolders(
        ...
        switch (timestampWatermarkMode) {
            case NO_TIMESTAMPS_WATERMARKS:
                {
                    for (Map.Entry<TopicRange, MessageId> partitionEntry :
     ...
     * offsets. Per-partition metric groups can be scoped by user variables.
     *
    ...
    private void registerOffsetMetrics(
            MetricGroup consumerMetricGroup, List<PulsarTopicState<T>> partitionOffsetStates) {

        for (PulsarTopicState<T> pts : partitionOffsetStates) {
            MetricGroup topicPartitionGroup =
                    consumerMetricGroup.addGroup(
                            OFFSETS_BY_TOPIC_METRICS_GROUP, pts.getTopicRange().getTopic());
    ...
    private static class OffsetGauge implements Gauge<MessageId> {
        ...
        public MessageId getValue() {
            switch (gaugeType) {
                ...
                default:
                    throw new IllegalStateException("Unknown gauge type: " + gaugeType);
            }
        }
    }
    // ------------------------------------------------------------------------
