...
import org.apache.flink.connectors.pulsar.common.{CachedPulsarClient, Logging, PulsarOptions, SchemaUtils, SourceSinkUtils}
...
class FlinkPulsarSink[T](
  ...
  @transient private val sendCallback = {
    if (doFailOnWrite) {
  ...
  protected def producerClose(): Unit = {
    ...
    if (singleProducer != null) {
      ...
      topic2Producer.foreach(_._2.clone())
    }
