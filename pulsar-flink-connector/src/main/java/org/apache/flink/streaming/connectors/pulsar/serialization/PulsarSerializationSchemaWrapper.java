...
public class PulsarSerializationSchemaWrapper<T>
        implements PulsarSerializationSchema<T>, PulsarContextAware<T> {
    ...
    public void serialize(T element, TypedMessageBuilder<T> messageBuilder) {
        ...
        if (deliverAtExtractor != null) {
            deliverAtExtractor
                    .apply(element)
                    .ifPresent(deliverAt -> messageBuilder.deliverAt(deliverAt));
        }
    ...
    public Schema<T> getSchema() {
            ...
            switch (schemaMode) {
                case SPECIAL:
                    ...
                    return new FlinkSchema<>(
                            schemaInfoWrapper.getSchemaInfo(), serializationSchema, null);
                case ATOMIC:
                    return new FlinkSchema<>(
                            SchemaTranslator.atomicType2PulsarSchema(dataType).getSchemaInfo(),
                            serializationSchema,
                            null);
                case POJO:
                    return new FlinkSchema<>(
                            SchemaUtils.buildSchemaForRecordClazz(clazz, recordSchemaType)
                                    .getSchemaInfo(),
                            serializationSchema,
                            null);
                case ROW:
                    return new FlinkSchema<>(
                            SchemaUtils.buildRowSchema(dataType, recordSchemaType),
                            serializationSchema,
                            null);
            }
        ...
        try {
            if (dataType instanceof AtomicDataType) {
                ...
                checkNotNull(
                        recordSchemaType, "for non-atomic type, you must set recordSchemaType");
                return new FlinkSchema<>(
                        SchemaUtils.buildRowSchema(dataType, recordSchemaType),
                        serializationSchema,
                        null);
    ...
    public static class Builder<T> {
        ...
        public PulsarSerializationSchemaWrapper.Builder<T> usePojoMode(
                Class<?> clazz, RecordSchemaType recordSchemaType) {
            checkArgument(mode == null, "you can only set one schemaMode");
        ...
        public PulsarSerializationSchemaWrapper.Builder<T> useRowMode(
                DataType dataType, RecordSchemaType recordSchemaType) {
            checkArgument(mode == null, "you can only set one schemaMode");
