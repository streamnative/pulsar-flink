 ...
 */
@Slf4j
public class PulsarRowDataSerializer implements SerializationSchema<RowData> {
    ...
    private org.apache.pulsar.client.api.Schema<Object> schema;

    private Function<Object, Object> converter;

    private SchemaType schemaType;

    public PulsarRowDataSerializer(DataType flinkType, boolean nullable, SchemaType schemaType) {
    ...
    public void open(InitializationContext context) throws Exception {
        try {
            SimpleSchemaTranslator schemaTranslator = new SimpleSchemaTranslator();
            this.schema = schemaTranslator.sqlType2PulsarSchema(flinkType);
            this.rootAvroType = schemaTranslator.sqlType2AvroSchema(flinkType);
            ...
            if (flinkType instanceof FieldsDataType) {
                ...
                BiFunction<PulsarSerializer.PositionedGetter, Integer, Object>
                        converter = singleValueConverter(flinkType, actualAvroType);
                baseConverter =
                        data -> converter.apply(new PulsarSerializer.PositionedGetter((Row) data), 0);
            }
    ...
    @Override
    public byte[] serialize(RowData flinkData) {
        final Object object = converter.apply(flinkData);
        return schema.encode(object);
    }

    private BiFunction<PulsarSerializer.PositionedGetter, Integer, Object> singleValueConverter(DataType dataType, Schema avroType) throws IncompatibleSchemaException {
    ...
    private Function<Object, Object> newStructConverter(FieldsDataType dataType, Schema avroStruct) throws IncompatibleSchemaException {
        ...
        List<BiFunction<PulsarSerializer.PositionedGetter, Integer, Object>> fieldConverters = new ArrayList<>();
        ...
        return row -> {
            ...
            for (int i = 0; i < numFields; i++) {
                if (rowX.getField(i) == null) {
                    ...
                    builder.set(pSchema.getFields().get(i), fieldConverters.get(i).apply(new PulsarSerializer.PositionedGetter(rowX), i));
                }
            }
    ...
    private BiFunction<PulsarSerializer.PositionedGetter, Integer, Object> newConverter(DataType dataType,
                                                                                        Schema avroType)
        ...
        if (tpe == LogicalTypeRoot.NULL && atpe == Schema.Type.NULL) {
            ...
            if (avroType.getLogicalType().equals(LogicalTypes.decimal(d.getPrecision(), d.getScale()))) {
            ...
            BiFunction<PulsarSerializer.PositionedGetter, Integer, Object> elementConverter =
            ...
            return (getter, ordinal) -> {
                ...
                for (int i = 0; i < len; i++) {
                    if (containsNull && arrayData[i] == null) {
                        ...
                        result[i] = elementConverter.apply(new PulsarSerializer.PositionedGetter(arrayData), i);
                    }
                }
            ...
            return (getter, ordinal) -> ((GenericAvroRecord) structConverter.apply(getter.getField(ordinal)))
                    .getAvroRecord();
        } else {
            throw new IncompatibleSchemaException(String.format(
                    "Cannot convert flink type %s to avro type %s", dataType.toString(), avroType.toString(true)));
