...
class PulsarTopicStateWithPeriodicWatermarks(
  topic: String,
    periodicWatermarks: AssignerWithPeriodicWatermarks[BinaryRow])
  ...
  def getTimestampForRow(row: BinaryRow, timestamp: Long): Long = {
...
class PulsarTopicStateWithPunctuatedWatermarks(
    topic: String,
    punctuatedWatermarks: AssignerWithPunctuatedWatermarks[BinaryRow])
  extends PulsarTopicState(topic) {

  /** The last watermark timestamp generated by this partition. */
  @volatile var partitionWatermark: Long = Long.MinValue

  def getTimestampForRow(row: BinaryRow, pulsarEventTimestamp: Long): Long = {
    punctuatedWatermarks.extractTimestamp(row, pulsarEventTimestamp)
  }

  def checkAndGetNewWatermark(row: BinaryRow, timestamp: Long): Watermark = {
